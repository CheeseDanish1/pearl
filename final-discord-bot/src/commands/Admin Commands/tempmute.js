...
module.exports.run = (client, message, args) => {
  ...
  if (!args[1])
    return message.channel.send(
      'Please specify an amount of time to mute the person for.\nExample: >TempMute @Cheese Danish 5m'
    );

  let time = args[1];

  if (!/^(\d+(m|h|s))+$/gi.test(time))
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      `${who} Was Muted by ${message.author.tag} for ${prettyMilliseconds(
        time,
        {verbose: true}
      )}.`
    )
  ...
  setTimeout(() => {
    ...
    message.channel.send({
      embed: {
        title: 'Unmute',
        description: `${who} Was Unmuted after ${prettyMilliseconds(time, {
          verbose: true,
        })}.`,
      },
    });
...
module.exports.info = {
  name: 'tempmute',
  alias: ['tmute'],
  usage: '<p>Tempmute [Member] [Time]',
  example: '<p>Tempmute @Jimmy#7932 3d',
  description: 'Temporary mute someone in your server',
  category: 'moderation',
};
