  ...
  if (arg == 'world') {
    try {
      const stayhome = api.all().then(response => {
        let covid = new Discord.MessageEmbed()
          ...
          .setImage(
            'https://i2.milimaj.com/i/milliyet/75/0x0/5e6e497aadcdeb15e8c11dcf.jpg'
          )
          .setThumbnail(
            'https://i2.milimaj.com/i/milliyet/75/0x0/5e6e497aadcdeb15e8c11dcf.jpg'
          );
    ...
    try {
      const stayhome2 = api.countries({country: arg}).then(response => {
        if (!response.cases)
          return message.channel.send('That country does not exist');
        let covid = new Discord.MessageEmbed()
          .setColor('#ff0000')
          .setTitle('#StayHome')
          .addField('Total Cases', response.cases, true)
...
module.exports.info = {
  name: 'covid',
  alias: [],
  usage: '<p>Covid [Country | World]',
  example: '<p>Covid France\n<p>Covid World',
  description: 'View covid information for a country or the world',
  category: 'misc',
};
