...
module.exports.run = async (client, message, args) => {
    ...
    if (person.bot) return message.channel.send(`This user is a bot, so does not have any stats`)

    let config =
      (await UserConfig.findOne({id: person.id})) ||
      (await UserConfig.create({id: person.id}));
	...
	ctx.fillText(`${person.username}'s Global Xp And Stuff,`, canvas.width / 4.0, 100);

	// Add an exclamation point here and below
	ctx.font = `26px sans-serif`
	ctx.fillStyle = '#ffffff';
    ctx.fillText(`Xp: ${xp}, Level: ${levelN}, Messages Sent: ${mes}, Rank: #${lev}`, canvas.width / 4.0, 180);
	...
	const attachment = new Discord.MessageAttachment(canvas.toBuffer(), 'gxp.png');
